...
void Actuator::setMotor(int speedL, int speedR) {
    if (_type == DIFFERENTIAL_DRIVE) {
        if (_pDriver[IDX_UWHEEL])
            _pDriver[IDX_UWHEEL]->setSpeed(speedL);
...
void Actuator::drive(int angle, int speed) {
    if (_type == DIFFERENTIAL_DRIVE) {
        ...
        _pDriver[IDX_UWHEEL]->setSpeed(speed);
        _pServo->write(angle);
    }
}
...
bool Actuator::getDelta(Ticks &a, Ticks &b, float *dtheta, float *ddist) {
    ...
    float dist_l = len_l * dlt.get().left  / _pDriver[IDX_UWHEEL]->getTPR();

    float len_r  = (_type == DIFFERENTIAL_DRIVE) ? (2 * M_PI * _pDriver[IDX_DWHEEL]->getRadius()) : len_l;
    float dist_r = (_type == DIFFERENTIAL_DRIVE) ? (len_r * dlt.get().right / _pDriver[IDX_DWHEEL]->getTPR()) : dist_l;
...
Ticks Actuator::getTicks(Ticks *ticks) {
    ...
    if (_type == STEERING) {
        t.set(millis(), _pDriver[IDX_UWHEEL]->getTicks(), _pDriver[IDX_UWHEEL]->getTicks());
    } else {
...
bool Actuator::updateOdometry() {
    ...
    if (_type == STEERING) {
        _ticks.set(millis(), _pDriver[IDX_UWHEEL]->getTicks(), _pDriver[IDX_UWHEEL]->getTicks());
    } else {
...
void Actuator::calibrate(int key) {
    ...
    switch (key) {
        ...
        case '/':
            ...
            rot  = delta.get().left / _pDriver[IDX_UWHEEL]->getTPR();
            rot  = rot * 60000 / delta.get().millis;
            LOG("wheel_l, ctr:%8ld, rpm:%6.1f\n", _ticks.get().left,  rot);

            if (_type == DIFFERENTIAL_DRIVE) {
                rot  = delta.get().right / _pDriver[IDX_DWHEEL]->getTPR();
                rot  = rot * 60000 / delta.get().millis;
